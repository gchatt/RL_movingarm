...
class Striatum(layers.Layer):
	def __init__(self, units=unit1*3,input_dim=15):
		...
		if use_constraints:
			self.w = self.add_weight(shape=(input_dim,units),initializer=w_init,trainable=True,constraint=non_neg())
		else:
			self.w = self.add_weight(shape=(input_dim,units),initializer=w_init,trainable=True)
		self.b = self.add_weight(shape=(units,), initializer=b_init, trainable=True)
...
class GPe(layers.Layer):
	def __init__(self,units=unit2*3,input_dim=unit1*3,str_dval=[]):
...
class STN(layers.Layer):
	def __init__(self,units=unit2*3,input_dim=unit2*3):
...
class GPi(layers.Layer):
	...
	def __init__(self,units=unit2*3,input_dim=(unit1+unit2)*3,str_dval=[]):
...
class Motor_thal(layers.Layer):
	def __init__(self,units=unit2*3,input_dim=(unit2*3)+unit3):
...
class Premotor(layers.Layer):
	def __init__(self,input_dim=9,l1_sz=unit3,l2_sz=unit3):
...
class Motor_cortex(layers.Layer):
	def __init__(self,l1_sz=unit3,l2_sz=unit3,action_sz=6):
...
class CTBG(keras.Model):
	def __init__(self,summary_writer,units):
		...
		self.dorsal_striatum = Striatum()
		#print(self.dorsal_striatum.trainable_weights[0])
		self.gpe = GPe(str_dval=self.dorsal_striatum.dval)
		self.stn = STN()
		self.gpi = GPi(str_dval=self.dorsal_striatum.dval)
		self.mthal = Motor_thal()
		self.premotor = Premotor()
		self.mctx = Motor_cortex()
	...
	def call(self,str_inputs,prem_inputs,reward,use_reward):
		...
		# if use_reward:
			# std_all = (self.std_all) / ((1+(reward/self.delta))**self.alpha)
			# std_str = (self.std_str) / ((1+(reward/self.delta))**self.alpha)
			# std_mc = (self.std_mc) / ((1+(reward/self.delta))**self.alpha)
		# elif not use_reward:
			# std_all = 0.01
			# std_str = 0.01
			# std_mc = 0.01
...
# print(c(x,p))
#str = Striatum()
#print(len(str.trainable_weights))
#print(len(str.non_trainable_weights))
#y = str(x)
#r = tf.nn.relu
#print(r(y))
